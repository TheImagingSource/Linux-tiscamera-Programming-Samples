...
TcamCamera::enable_video_display(GstElement *displaysink)
{
    ...
    gst_bin_add_many(GST_BIN(displaybin_), queue, videoscale_, convert, capsfilter, displaysink, nullptr);
    if (!gst_element_link_many(tee_, queue, videoscale_, convert, capsfilter, displaysink, nullptr))
